...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        return view('employerApp.auth.login');
    }
    ...
    protected function authenticated(Request $request, $user)
    {
        if(!empty($user))
        {
            $user_login = UserLoginLog::where('user_id', $user['id'])->where('user_type', 'employer')->orderBy('created_at','desc')->first();

            if(!empty($user_login))
            {
                  $specialist = Employer::find($user['id']);
                  $specialist->last_login = $user_login->created_at;
                  $specialist->save();
                  UserLoginLog::create(['user_id'=>$user['id'], 'user_type' =>'employer']);  
            }
            else
            {
               $dta =  UserLoginLog::create(['user_id'=>$user['id'], 'user_type' =>'employer']);
