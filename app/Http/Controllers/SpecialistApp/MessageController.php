...
class MessageController extends Controller
{
    ...
    public function allThreads()
    {
        ...
        $this->data['allThreads']  = MessageRoom::with(['job','candidate','employer','admin'])->orderBy('id', 'DESC')->where(['spc_id'=>$my_id,'deleted_by_spc'=>null])->paginate(4);

        return view('specialistApp.messages.all-threads',$this->data);
    }
    ...
    {   
        try
        {
            ...
            return view('specialistApp.messages.chat',$this->data);
        } 
        catch (\Exception $e) 
        {
           dd($e->getMessage());
    ...
    { 
        ...
        if(!empty($checkThread))
        {
            ...
            $messageConversation->sender_type = 'specialist';
            if(!empty($adminId))
            {
    ...
    public function deleteThread($id)
    {
         ...
         if($messageRoom){
            $messageRoom->deleted_by_spc = date("Y-m-d");
            if($messageRoom->save())
            {
