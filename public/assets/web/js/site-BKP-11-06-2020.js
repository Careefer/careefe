...
if($('#drag-drop3').length > 0)
{
  ...
  var myDropZone = new Dropzone('#drag-drop3',{ 
      paramName:'resume',
      url: site_url+'/candidate/resume',
      addRemoveLink:true,
      autoProcessQueue: false,
      ...
      init: function () {
        var myDropZone = this;
        this.on('addedfile',function(file){
          $('#browsedresumeHere').html(file.name);
        });
        this.on('sending', function(file, xhr, formData) {
  ...
  var myDropZone1 = new Dropzone('#cover_letter',{ 
    paramName:'cover_letter',
    url: site_url+'/candidate/cover_letter',
    addRemoveLinks:true,
    autoProcessQueue: false,
    // previewsContainer:"div .dropzone-previews",
    uploadMultiple:false,
  ...
  {
    e.preventDefault();
    URL = $(".profile-main-form").attr('action');
        ...
        {   
          ...
          success: function(result){
            ...
            if(result.status == "success"){
              ...
              $('.spc-profile-form-msg').html('<h4 class="success_msg">'+result.msg+'</h4>');

              // fetch the useid 
              var userid = result.user_id;
              $(".profile-main-form").append('<input type="hidden" class="userid" name="userid" id="userid" value="">');
