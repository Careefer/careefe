...
<div class="ref-info">
  ...
  <ul class="ref-list clearfix" style="margin-bottom: 10px; ">
    <li>
  ...
  <div class="payment-status clearfix">
    ...
    <div class="rate-app clearfix">
      <span class="rate-text">Rate Applicant:</span>
      <!-- Rating Stars Box -->
        <div class='rating-stars'>
          <ul id='stars'>
            <li class='star {{ ($app->rating_by_referee >0 && $app->rating_by_referee >= 1 ) ? "selected": "" }}' title='Poor' data-value='1' data-appId="{{ $app->id }}">
              <i class='fa fa-star fa-fw'></i>
            </li>
            <li class='star {{ ($app->rating_by_referee >0 && $app->rating_by_referee >= 2 ) ? "selected": "" }}' title='Fair' data-value='2' data-appId="{{ $app->id }}">
              <i class='fa fa-star fa-fw'></i>
            </li>
            <li class='star {{ ($app->rating_by_referee >0 && $app->rating_by_referee >= 3 ) ? "selected": "" }}' title='Good' data-value='3' data-appId="{{ $app->id }}">
              <i class='fa fa-star fa-fw'></i>
            </li>
            <li class='star {{ ($app->rating_by_referee >0 && $app->rating_by_referee >= 4 ) ? "selected": "" }}' title='Excellent' data-value='4' data-appId="{{ $app->id }}">
              <i class='fa fa-star fa-fw'></i>
            </li>
            <li class='star {{ ($app->rating_by_referee >0 && $app->rating_by_referee >= 5 ) ? "selected": "" }}' title='WOW!!!' data-value='5' data-appId="{{ $app->id }}">
              <i class='fa fa-star fa-fw'></i>
            </li>
          </ul>
        </div>
  ...
  <ul class="application-btn">
    <li>
      @php
         $canId =  my_id();
         $room_data = ['jobId'=>$job->id,'appId'=>$app->id,'canId'=>$canId,'spcId'=>$app->specialist_id];
         $roomId =  base64_encode(json_encode($room_data));
      @endphp
      <a href="{{url('candidate/chat/'.$roomId)}}" class="msg-specialist">Message Specialist</a>
    </li>
    <li>
      <a href="{{ asset('storage/candidate/resume/'.$app->resume) }}" class="button-link resume-btn">View CV</a>
    </li>
  </ul>
</div>
</li>
...
function responseMessage(application_id, rating)
{
  if(application_id && rating)
  {
    $.ajax({
            type: "POST",
            cache: false,
            url: "{{ route('candidate.update-application-rating') }}",
            dataType: "json",
